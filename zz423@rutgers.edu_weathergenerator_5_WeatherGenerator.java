...
public class WeatherGenerator {
    ...
    public static int[] oneMonthGenerator(double longitute, double latitude, int month, double[][] drywet,
        ...
        for (int r = 0; r < drywet.length; r++) {
            for (int c = 0; c < drywet[r].length; c++) {

                if (drywet[r][0] == longitute && drywet[r][1] == latitude) {
                    pDrywet = drywet[r][month];
                }

                if (wetwet[r][0] == longitute && wetwet[r][1] == latitude) {
                    pWetwet = wetwet[r][month];
                }
            }
        }

        int days = numberOfDaysInMonth[month - 2];
        int[] forecast = new int[days];
        double firstDay = StdRandom.uniform();
        if (firstDay <= 0.49 && firstDay >= 0.0)
            forecast[0] = WET;
        else
            forecast[0] = DRY;
    ...
    public static int lengthOfLongestSpell(int[] forecast, int mode) {
        ...
        } int max = Math.max(count, largest);
